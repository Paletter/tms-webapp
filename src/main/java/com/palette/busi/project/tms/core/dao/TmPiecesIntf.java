...
public interface TmPiecesIntf extends BaseMybatisMapper {
	...
	@Insert("insert into tm_pieces ( TM_PIECES_ID, WAREHOUSE_CODE, COUNTRY_CODE, MEMBER_CODE, PIECES_NO, LOGISTICS_NO, ACTUAL_WEIGHT, VOLUME_WEIGHT, LENGTH, WIDTH, HEIGHT, CHECK_DATE, MEMO, DELIVERY_DATE, IS_BIND_DELIVERY, TM_CONSIGNMENT_ID, CONSIGNMENT_NO, SERVICE_PRODUCT_CODE, UNIT_NO, TM_UNIT_ID, SECTOR_NO, TM_SECTOR_ID, DELIVERY_VENDOR_CODE, DELIVERY_NO, COMPANY_CODE, RECORD_VERSION, CREATE_USER_CODE, CREATE_DATE_TIME, UPDATE_USER_CODE, UPDATE_DATE_TIME ) values (#{tmPiecesId},#{warehouseCode},#{countryCode},#{memberCode},#{piecesNo},#{logisticsNo},#{actualWeight},#{volumeWeight},#{length},#{width},#{height},#{checkDate},#{memo},#{deliveryDate},#{isBindDelivery},#{tmConsignmentId},#{consignmentNo},#{serviceProductCode},#{unitNo},#{tmUnitId},#{sectorNo},#{tmSectorId},#{deliveryVendorCode},#{deliveryNo},#{companyCode},#{recordVersion},#{createUserCode},#{createDateTime},#{updateUserCode},#{updateDateTime})")
